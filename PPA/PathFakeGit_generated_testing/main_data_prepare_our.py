...
raw_tokenized_all_examples = load_data(100000, 'data/path15_texts.txt', True, "spacy")
...
data_path = '220913__new_data/gen_test1516.json'
label_dic = get_label_dict('data/labelTwitter15.txt')
with tqdm(open(data_path, "r"), desc=f"loading {data_path}") as f:
    for line in f:
        example = json.loads(line.strip())
        tokenized_examples = []
        id_ = example['id_']
        if str(id_) not in label_dic.keys():
            continue
...
with open('220913__new_data/gen_test15_dict_random.json', 'w') as f:
    json.dump(random_id2paths_dict, f)

with open('220913__new_data/gen_test15_dict_response.json', 'w') as f:
    json.dump(response_id2paths_dict, f)



# 读取全部回复文本（1000000是上限；如果文件中超过上限，则后面的不处理，用于处理数据过多的文件，100000手动设置）
raw_tokenized_all_examples = load_data(100000, 'data/path16_texts.txt', True, "spacy")
examples_num = len(raw_tokenized_all_examples)
# 按照比例划分作为训练和验证集
# 复制一遍；便于打乱顺序等操作
random_all_examples = [line for line in raw_tokenized_all_examples]
...
random_id2paths_dict = {}
data_path = '220913__new_data/gen_test1516.json'
label_dic = get_label_dict('data/labelTwitter16.txt')
with tqdm(open(data_path, "r"), desc=f"loading {data_path}") as f:
    for line in f:
        example = json.loads(line.strip())
        tokenized_examples = []
...
with open('220913__new_data/gen_test16_dict_random.json', 'w') as f:
    json.dump(random_id2paths_dict, f)

with open('220913__new_data/gen_test16_dict_response.json', 'w') as f:
    json.dump(response_id2paths_dict, f)
