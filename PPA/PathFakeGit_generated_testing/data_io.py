...
def load_path_data_with_1516(train_data_path, test_data_path, label_path, tmp_dict_save_path, npz_path):
    # 获取标签字典
    label_dic15 = get_label_dict(label_path)
    label_dic16 = get_label_dict(label_path.replace('Twitter15', 'Twitter16'))
    label_dic = {**label_dic15, **label_dic16}
    print("loading tree label:", len(label_dic))
    if not os.path.exists(tmp_dict_save_path):
        # 从本地读取文本数据的id形式
        print('load sparse matrix...')
        ...
        print('dict already exists,load from', tmp_dict_save_path)
        with open(tmp_dict_save_path, 'r') as f:
            id2paths_dict15 = json.load(f)
        with open(tmp_dict_save_path.replace('Twitter15', 'Twitter16'), 'r') as f:
            id2paths_dict16 = json.load(f)
    id2paths_dict = {**id2paths_dict15, **id2paths_dict16}
    # 加载训练集id并转换成模型的输入x,y
    l1, l2, l3, l4 = 0, 0, 0, 0
    x_train, y_train = [], []
    per_class_threshold = int(_args.train_threshold / _args.class_num)
    ...
    per_class_threshold = int(_args.train_threshold / _args.class_num)
    with open(train_data_path.replace('Twitter15', 'Twitter16'), 'r') as f:
        train_ids = [line.strip() for line in f.readlines()]
        # random.shuffle(train_ids)
        for id in train_ids:
            y, l1, l2, l3, l4 = loadLabel(label_dic[id], l1, l2, l3, l4)
    ...
    with open(test_data_path.replace('Twitter15', 'Twitter16'), 'r') as f:
        train_ids = [line.strip() for line in f.readlines()]
        for id in train_ids:
            y, l1, l2, l3, l4 = loadLabel(label_dic[id], l1, l2, l3, l4)
            y_test.append(y)
...
def load_path_data_with_1516_test(test_data_path, label_path, tmp_dict_save_path, npz_path):
    # 获取标签字典
    label_dic = get_label_dict(label_path)
    print("loading tree label:", len(label_dic))
    if not os.path.exists(tmp_dict_save_path):
    ...
    x_test, y_test = [], []
    with open(test_data_path, 'r') as f:
        train_ids = [json.loads(line.strip())['id_'] for line in f.readlines()]
    for id in train_ids:
        if id not in id2paths_dict.keys():
            continue
        y, l1, l2, l3, l4 = loadLabel(label_dic[id], l1, l2, l3, l4)
        y_test.append(y)
        x_test.append(id2paths_dict[id])
...
def load_path_data_with_1516_test_with_gen(test_data_path, label_path, tmp_dict_save_path):
    # 获取标签字典
    label_dic = get_label_dict(label_path)
    print("loading tree label:", len(label_dic))
    with open(tmp_dict_save_path, 'r') as f:
        id2paths_dict15 = json.load(f)
    with open(tmp_dict_save_path.replace('15', '16'), 'r') as f:
        id2paths_dict16 = json.load(f)
    id2paths_dict = {**id2paths_dict15, **id2paths_dict16}
...
def load_path_data_pheme_with_test(test_data_path, label_path, tmp_dict_save_path, npz_path):
    # 获取标签字典
    label_dic = get_label_dict(label_path)
    print("loading tree label:", len(label_dic))
    if not os.path.exists(tmp_dict_save_path):
...
def load_path_data_pheme_with_test_gen(test_data_path, label_path, tmp_dict_save_path):
    with open(tmp_dict_save_path, 'r') as f:
        id2paths_dict = json.load(f)
    label_dic = get_label_dict(label_path)
    # 加载测试集id并转换成模型的输入x,y
    ...
    with open(test_data_path, 'r') as f:
        for line in f.readlines():
            ...
            id = str(line['id_'])
            y, l1, l2, l3 = loadLabelPheme(label_dic[id], l1, l2, l3)
            # y_test[label_dic[id]] = y_test.get(label_dic[id], []) + [y]
            # x_test[label_dic[id]] = x_test.get(label_dic[id], []) + [id2paths_dict[id]]
            y_test.append(y)
            x_test.append(id2paths_dict[id])
    print("loading test set:", l1, l2, l3)
    return x_test, y_test
